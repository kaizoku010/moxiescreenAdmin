...
// device one
    ...
    const unsubscribeBattery2 = onValue(batteryRef2, (snapshot) => {
      const batteryData = snapshot.val();
      setDevic2Data(prevData => ({ ...prevData, battery: batteryData }));
      console.log("Real-time battery data from device 2:", batteryData);
    });

    const unsubscribeStatus2 = onValue(connectionRef2, (snapshot) => {
      const connection = snapshot.val();
      setDevic2Data(prevData => ({ ...prevData, connection: connection }));
      console.log("Real-time battery data:", connection);
    });

    // device 3
    
    const unsubscribeLocation3 = onValue(locationRef2, (snapshot) => {
      const locationData = snapshot.val();
      setDevic2Data(prevData => ({ ...prevData, location: locationData }));
      console.log("Real-time location data:", locationData);
    });

    const unsubscribeBattery3 = onValue(batteryRef2, (snapshot) => {
      const batteryData = snapshot.val();
      setDevic2Data(prevData => ({ ...prevData, battery: batteryData }));
      console.log("Real-time battery data from device 2:", batteryData);
    });

    const unsubscribeStatus3 = onValue(connectionRef2, (snapshot) => {
      const connection = snapshot.val();
      setDevic2Data(prevData => ({ ...prevData, connection: connection }));
      console.log("Real-time battery data:", connection);
    });
