...
import { database, ref, get, push } from './firebaseConfig';
...
const CampaignDetails = () => {
  ...
  const handlePlaylistSelection = (selectedPlaylist) => {
    // Save the selected playlist to Firebase under the campaign
    const campaignRef = ref(database, `campaigns/${campaignId}`);
    try {
      push(campaignRef.child('playlists'), selectedPlaylist);
  ...
  return (
    <div className="campaign-details-container">
      ...
      <Playlist onPlaylistSelect={handlePlaylistSelection} />
    </div>
  );
};
