...
function AdList({ title, thumb, date, size, type }) {
  ...
  useEffect(() => {
    let newSocket = ""; 
    const socketDataPull = async()=>{
    newSocket = await io('https://socket-yzb8.onrender.com', { transports: ["websocket"] });
   await newSocket.on('location', handleDataReceived);
